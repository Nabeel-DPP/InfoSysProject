...
export default function Rights() {
  ...
  useEffect(() => {
    const fetchAreas = async () => {
      try {
        const response = await axios.get("http://localhost:5555/rights");
  ...
  const handleEditClick = (id) => {
    ...
    navigate('/rightsEdit', { state: { rowData: selectedRow } });
  };
...
const confirmDelete = async () => {
  if (recordToDelete) {
    try {
      ...
      await axios.delete(`http://localhost:5555/rights/${recordToDelete}`); // Adjust to your delete endpoint
...
<div className="add-btn d-flex w-100 mb-5 justify-content-end">
    <Link to="/rightsInsert" className='btn  btn-outline-success' > <i className="fa-regular fa-pen-to-square"></i></Link>
