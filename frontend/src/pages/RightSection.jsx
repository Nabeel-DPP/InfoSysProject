...
export default function RightSection() {
  ...
  useEffect(() => {
    const fetchAreas = async () => {
      try {
        const response = await axios.get("http://localhost:5555/rtSection");
  ...
  const handleEditClick = (id) => {
    ...
    navigate('/rtEdit', { state: { rowData: selectedRow } });
  };
...
const confirmDelete = async () => {
  if (recordToDelete) {
    try {
      ...
      await axios.delete(`http://localhost:5555/rtSection/${recordToDelete}`); // Adjust to your delete endpoint
...
<div className="add-btn d-flex w-100 mb-5 justify-content-end">
    <Link to="/rtInsert" className='btn  btn-outline-success' > <i className="fa-regular fa-pen-to-square"></i></Link>
   
    
    </div>
    <div className="table-caption">
    <h3 className="text-center col-md-6 border form-head-text p-2">Rights Section List</h3>
    </div>
